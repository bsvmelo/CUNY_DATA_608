...
server <- function(input, output, session) {
  ...
  output$plot11 <- renderPlot({ place_plot11 () })
   ...
   dataforplot_12 <- reactive({ dt1 <- df %>% filter(Pf_category %in% input$dist) %>% select(-fund_strategy)
                                dt1$Pf_category <- factor(dt1$Pf_category,levels =c('Aggressive','Moderate','Conservative'))
                                return(dt1)})
  
  place_plot12<- reactive ({
    ggplot(df%>%filter(fund_strategy %in% cat.pf), aes(x=fund_stdev_3years,y=round(fund_return_3years*100,0)))+geom_point(data=dataforplot_12(),colour = "grey70",alpha=1, size=2) + geom_point(aes(colour = fund_strategy),alpha=1, size=2) +
